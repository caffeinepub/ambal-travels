...
import type { Booking as _Booking, Status as _Status, Time as _Time, UserProfile as _UserProfile, UserRole as _UserRole } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async markBookingCompleted(arg0: bigint): Promise<void> {
        if (this.processError) {
            try {
    ...
    async submitBooking(arg0: New): Promise<bigint> {
        if (this.processError) {
            try {
    ...
    async adminLogin(email: string, password: string): Promise<string | null> {
        const result = await this.actor.adminLogin(email, password);
        return result.length === 0 ? null : result[0];
    }
    async validateAdminSession(token: string): Promise<boolean> {
        return this.actor.validateAdminSession(token);
    }
    async adminLogout(token: string): Promise<void> {
        return this.actor.adminLogout(token);
    }
    async getAllBookingsWithToken(token: string): Promise<Array<Booking>> {
        const result = await this.actor.getAllBookingsWithToken(token);
        return from_candid_vec_n3(this._uploadFile, this._downloadFile, result);
    }
    async deleteBookingWithToken(token: string, bookingId: bigint): Promise<void> {
        return this.actor.deleteBookingWithToken(token, bookingId);
    }
    async markBookingCompletedWithToken(token: string, bookingId: bigint): Promise<void> {
        return this.actor.markBookingCompletedWithToken(token, bookingId);
    }
}
...
function from_candid_variant_n7(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return "pending" in value ? Status.pending : "completed" in value ? Status.completed : value;
}
